...
export default function AIPage() {
  ...
  const {
    ...
    onFinish: async (prompt: string, completion: string) => {
      ...
      if (agent && !isConversation && accessToken && newConversation) {
  ...
  const fetchMessages = async (chatId: string, accessToken: string) => {
    ...
    const data = await getMessages(chatId, accessToken)
  ...
  useEffect(() => {
    if (chatId && accessToken) {
      ...
      fetchMessages(chatId, accessToken)
    }
  }, [chatId, accessToken])
  ...
  return (
    <main className='min-w-screen min-h-screen text-[#f5f5f5] relative flex flex-col'>
      {/* Header */}
