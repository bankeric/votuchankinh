...
import { getAgents, getConversations } from '@/services/aiService'
import { Agent, Section } from '@/types'
...
export default function AIPage() {
  ...
  // Agents
  const [agent, setAgent] = useState('giac-ngo')
  ...
  const fetchConversations = async (accessToken: string) => {
    const data = await getConversations(undefined, undefined, accessToken)
  ...
  const fetchAgents = async (language: 'vi' | 'en', accessToken: string) => {
    if (!accessToken) return
    const data = await getAgents(undefined, language, accessToken)
  ...
  useEffect(() => {
    ...
    fetchAgents(language, accessToken)
    ...
    <main className='min-h-screen text-[#f5f5f5] relative flex flex-col'>
       ...
       hover:bg-[#efe2c9] transition-colors'
        ...
        <div className='flex items-center space-x-2 md:space-x-3'>
          ...
          >
            ...
            {/* <option value='giac-ngo'>Giác Ngộ</option>
            <option value='tam-an'>Tâm An</option>
            <option value='ke-van'>Kệ Vấn</option>
            <option value='ngo'>Ngộ</option> */}
          </select>
        </div>
      </header>
      ...
      <div className='flex flex-1 pt-16'>
        ...
        <div className='flex-1 flex flex-col min-h-0 w-full'>
                    ...
                    {message.role === 'assistant' ? (
                      <div className='flex items-start space-x-2 w-full max-w-[90%] md:max-w-[80%]'>
                        ...
                        <div className='flex-1 p-3 md:p-4 rounded-2xl shadow-lg backdrop-blur-sm bg-[#f3ead7] text-[#1f1f1f] border-2 border-[#2c2c2c] shadow-[0_2px_0_#00000030,0_0_0_3px_#00000010_inset]'>
                          <p className='font-serif leading-relaxed text-sm md:text-base font-medium break-words'>
                            {message.content}
                          </p>
          ...
          {/* Input Area */}
          {/* <div className="border-t border-[#991b1b]/20 bg-background/30 backdrop-blur-sm p-2 md:p-4">
            <div className="max-w-full mx-auto">
              <div className="flex space-x-2 md:space-x-4">
                ...
                <input
                  ...
                  className="flex-1 bg-[#f3ead7] text-[#1f1f1f] placeholder-[#1f1f1f]/60 font-serif rounded-2xl px-3 py-2 md:px-6 md:py-3 text-sm md:text-base
                 border-2 border-[#2c2c2c] shadow-[0_2px_0_#00000030,0_0_0_3px_#00000010_inset]
                 hover:bg-[#efe2c9] focus:bg-[#efe2c9] focus:outline-none transition-all duration-300"
                  disabled={isTyping}
                />
                <button
