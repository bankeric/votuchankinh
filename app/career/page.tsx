...
import { ChevronDown, Mail, ArrowLeft, Sparkles } from "lucide-react"
...
export default function CareerPage() {
  ...
  return (
    <div className="min-h-screen bg-[#EFE0BD] text-[#2c2c2c] font-serif">
      {/* Header */}
      <div className="container mx-auto px-4 py-4">
        ...
        <div className="text-center mb-8 animate-fade-in">
          <div className="flex items-center justify-center gap-2 mb-2">
            ...
            <h1 className="text-2xl md:text-3xl font-bold text-[#991b1b]">{currentContent.title}</h1>
            <Sparkles className="w-5 h-5 text-[#991b1b]" />
          </div>
          <p className="text-base md:text-lg text-[#991b1b]/80 mb-3 font-light italic">{currentContent.subtitle}</p>
          <p className="text-sm md:text-base text-[#2c2c2c]/70 max-w-3xl mx-auto leading-relaxed">
            {currentContent.intro}
          </p>
        </div>

        {/* Career/Volunteer Toggle Buttons */}
        <div className="max-w-4xl mx-auto mb-6">
          <div className="flex items-center justify-center gap-2 bg-white/50 border-2 border-[#991b1b] rounded-full p-1 w-fit mx-auto shadow-[0_2px_0_#991b1b30]">
            <button
              onClick={() => {
        ...
        <div className="max-w-4xl mx-auto mb-8">
          <div className="space-y-3">
            {currentRoles.map((role, index) => (
              <div
                key={index}
                className="bg-white/50 border-2 border-[#991b1b] rounded-xl overflow-hidden shadow-[0_2px_0_#991b1b30,0_0_0_2px_#991b1b10_inset] hover:shadow-[0_4px_0_#991b1b40,0_0_0_2px_#991b1b15_inset] transition-all duration-200"
              >
                {/* Card Header - Always Visible */}
                <button
                  onClick={() => setExpandedRole(expandedRole === index ? null : index)}
                  className="w-full p-4 text-left hover:bg-[#991b1b]/5 transition-colors duration-200"
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1">
                      <h3 className="text-lg md:text-xl font-bold text-[#2c2c2c] mb-1">{role.title}</h3>
                      <p className="text-sm text-[#2c2c2c]/70 mb-2">{role.summary}</p>
                      <div className="flex items-center gap-2 text-xs text-[#991b1b] font-semibold">
                        <span className="bg-[#991b1b]/10 px-2 py-0.5 rounded-full">{role.experience}</span>
                        <span className="bg-[#991b1b]/10 px-2 py-0.5 rounded-full">
                          {language === "en" ? "Remote" : "Từ xa"}
                        </span>
                      </div>
                    </div>
                    <ChevronDown
                ...
                >
                  <div className="px-4 pb-4 border-t border-[#991b1b]/20">
                    ...
                    <div className="mt-4 mb-4">
                      <h4 className="text-xs font-bold text-[#991b1b] uppercase tracking-wide mb-2">
                        {language === "en" ? "Tech Stack" : "Công nghệ"}
                      </h4>
                      <div className="flex flex-wrap gap-1.5">
                        {role.techStack.map((tech, techIndex) => (
                          <span
                            key={techIndex}
                            className="bg-[#2c2c2c] text-[#d4af37] px-2 py-1 rounded text-xs font-mono font-semibold border border-[#d4af37]/30 hover:border-[#d4af37] transition-colors duration-200"
                          >
                            {tech}
                          </span>
                        ))}
                    ...
                    {/* Responsibilities */}
                    <div className="mb-4">
                      <h4 className="text-sm font-bold text-[#2c2c2c] mb-2">
                      ...
                      <ul className="space-y-1.5">
                        {role.responsibilities.map((resp, respIndex) => (
                          <li
                            key={respIndex}
                            className="text-[#2c2c2c]/80 text-xs leading-relaxed flex items-start gap-2"
                          >
                            <span className="text-[#991b1b] mt-1 flex-shrink-0">•</span>
                            <span>{resp}</span>
                          </li>
                        ))}
                      </ul>
                    ...
                    <div>
                      <h4 className="text-sm font-bold text-[#2c2c2c] mb-2">
                        {language === "en" ? "Qualifications" : "Yêu cầu"}
                      </h4>
                      <ul className="space-y-1.5">
        ...
        <div className="max-w-3xl mx-auto pb-8">
          <div className="bg-white/50 border-2 border-[#991b1b] rounded-xl p-5 shadow-[0_2px_0_#991b1b30,0_0_0_2px_#991b1b10_inset] text-center">
            <h2 className="text-xl md:text-2xl font-bold text-[#2c2c2c] mb-3">{currentApply.title}</h2>
            <p className="text-sm text-[#2c2c2c]/80 mb-3">{currentApply.description}</p>
            <a
              href={`mailto:${currentApply.email}`}
              className="inline-flex items-center gap-2 bg-[#991b1b] text-[#f6efe0] px-5 py-2 rounded-full text-sm font-semibold hover:bg-[#7a1515] transition-colors duration-200 mb-3"
            >
              <Mail className="w-4 h-4" />
              {currentApply.email}
            </a>
            <p className="text-xs text-[#2c2c2c]/70 italic">{currentApply.note}</p>
          </div>
        </div>
      </div>
    </div>
