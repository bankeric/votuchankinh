...
export const getToolCallResoning = (toolCall: string, language: Language) => {
  const text = language === Language.VI ? 'AI muốn thực hiện hành động sau:' : 'AI wants to perform the following action:';
  return `${text} "${toolCalls[toolCall as keyof typeof toolCalls][language]}"`;
};

const listOutObjectsRecursively = (objects: Record<string, any> | string): string => {
  if (typeof objects === 'string') {
    return objects;
  }
  return Object.keys(objects).map(key => {
    if (typeof objects[key] === 'object' && objects[key] !== null) {
      return `- **${key.charAt(0).toUpperCase() + key.slice(1).toLowerCase()}**: ${listOutObjectsRecursively(objects[key])}`;
    }
    return `- **${key.charAt(0).toUpperCase() + key.slice(1).toLowerCase()}**: ${objects[key]}`;
  }).join("\n");
}
  ...
  .map(
    (key) =>
      `- **${key.charAt(0).toUpperCase() + key.slice(1).toLowerCase()}**: ${
        listOutObjectsRecursively(request.approval_request.arguments[key])
      }`
    )
    .join("\n");
};
...
export const getJobState = (status: string, language: Language) => {
  switch (status) {
    case "JOB_STATE_UNSPECIFIED":
      return language === Language.VI ? "Trạng thái công việc không xác định" : "The job state is unspecified.";
    case "JOB_STATE_QUEUED":
      return language === Language.VI ? "Công việc đã được tạo hoặc tiếp tục và chưa bắt đầu xử lý" : "The job has been just created or resumed and processing has not yet begun.";
    case "JOB_STATE_PENDING":
      return language === Language.VI ? "Dịch vụ đang chuẩn bị chạy công việc" : "The service is preparing to run the job.";
    case "JOB_STATE_RUNNING":
      return language === Language.VI ? "Công việc đang được thực hiện" : "The job is in progress.";
    case "JOB_STATE_SUCCEEDED":
      return language === Language.VI ? "Công việc đã hoàn thành thành công" : "The job completed successfully.";
    case "JOB_STATE_FAILED":
      return language === Language.VI ? "Công việc đã thất bại" : "The job failed.";
    case "JOB_STATE_CANCELLING":
      return language === Language.VI ? "Công việc đang bị hủy" : "The job is being cancelled.";
    case "JOB_STATE_CANCELLED":
      return language === Language.VI ? "Công việc đã bị hủy" : "The job has been cancelled.";
    case "JOB_STATE_PAUSED":
      return language === Language.VI ? "Công việc đã bị tạm dừng" : "The job has been paused.";
    case "JOB_STATE_EXPIRED":
      return language === Language.VI ? "Công việc đã hết hạn" : "The job has expired.";
    case "JOB_STATE_UPDATING":
      return language === Language.VI ? "Công việc đang được cập nhật" : "The job is being updated.";
    case "JOB_STATE_PARTIALLY_SUCCEEDED":
      return language === Language.VI ? "Công việc đã hoàn thành một phần" : "The job is partially succeeded, some results may be missing due to errors.";
    default:
      return language === Language.VI ? "Trạng thái công việc không xác định" : "The job state is unspecified.";
  }
};
...
/**
 * Creates an audio element with safe blob URL handling
 * @param blob - The audio blob
 * @param onEnd - Callback when audio ends
 * @param onError - Callback when audio errors
