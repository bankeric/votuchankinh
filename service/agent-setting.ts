...
import { AgentSetting, CreateAgentSettingDto, UpdateAgentSettingDto, AgentSettingQueryParams } from '@/interfaces/agent-setting';
...
class AgentSettingService {
  ...
  async getById(uuid: string): Promise<AgentSetting> {
    try {
      const { data } = await axiosInstance.get<AgentSetting>(`${this.BASE_URL}/${uuid}`);
      return data;
    } catch (error) {
  ...
  async create(settingData: CreateAgentSettingDto): Promise<AgentSetting> {
    try {
      const { data } = await axiosInstance.post<AgentSetting>(this.BASE_URL, settingData);
      return data;
    } catch (error) {
  ...
  async update(uuid: string, settingData: UpdateAgentSettingDto): Promise<AgentSetting> {
    try {
      const { data } = await axiosInstance.put<AgentSetting>(`${this.BASE_URL}/${uuid}`, settingData);
      return data;
    } catch (error) {
  ...
  async getByAgentId(agentId: string, params?: { limit?: number; offset?: number }): Promise<AgentSetting[]> {
    try {
      const queryParams = new URLSearchParams();
      ...
      const { data } = await axiosInstance.get<AgentSetting[]>(`${this.BASE_URL}?${queryParams.toString()}`);
      return data;
    } catch (error) {
      console.error(`Error fetching agent settings for agent ${agentId}:`, error);
      throw error;
    }
  }
}
