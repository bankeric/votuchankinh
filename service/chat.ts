...
class ChatService {
  ...
  // Get all chats
  async getAll(
    page: number = DEFAULT_CHAT_PAGE
  ): Promise<{ data: Chat[]; totalCount: number }> {
    ...
    try {
      const { data, headers } = await axiosInstance.get<Chat[]>(
        `${this.BASE_URL}?offset=${offset}&limit=${limit}`
      )
