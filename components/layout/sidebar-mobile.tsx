...
export const SidebarMobile = ({
  ...
  return (
    <>
      ...
      {shouldRenderLayer && (
        <div
          className={`fixed inset-0 z-40 md:hidden transition-opacity duration-250 ease-out ${isMobileOpen ? 'opacity-100 bg-black/50' : 'opacity-0 bg-black/0'
            }`}
      ...
      {shouldRenderLayer && (
        <div
          className={`fixed left-0 top-0 h-full w-80 bg-[#efe0bd] border-r  border-[#2c2c2c]/30 flex flex-col z-50 md:hidden transform transition-transform duration-300 ease-in-out ${isMobileOpen ? 'translate-x-0' : '-translate-x-full'
            }`}
        >
          {/* Header */}
