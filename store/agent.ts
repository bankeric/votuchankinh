...
import { appToast, appToastLoading, appToastUpload, handleUploadEvent } from "@/lib/toastify";
...
interface AgentState {
  ...
  createAgent: (agentData: Partial<CreateAgentDto>, language: string) => Promise<Agent | null>;
  updateAgent: (
    uuid: string,
  ...
  removeFile: (fileId: string, agentId: string, corpusId?: string) => Promise<void>;
}
...
export const useAgentStore = create<AgentState>()((set, get) => {
  const setSelectedAgentId = (agentId: string | null, isGetFiles: boolean = true) => {
    if (!agentId) {
      set({ selectedAgentId: null, previewMessages: [], currentSessionId: null });
      return;
    }
    set({ selectedAgentId: agentId, previewMessages: [], currentSessionId: uuidv4() });
    const { agentsMap } = get();
  ...
  const getFiles = async (agentId: string, corpusId?: string): Promise<RagFile[]> => {
    set({ isLoading: true });
  ...
  const uploadFiles = async (agentId: string, files: File[]) => {
    ...
    const response = await agentService.uploadFiles(agentId, files, (update) => {
  ...
  const removeFile = async (fileId: string, agentId: string, corpusId?: string) => {
  ...
  return {
        ...
        if (state.formData.conversationStarters.length < 4) {
          return {
            formData: {
              ...state.formData,
              conversationStarters: [
                ...state.formData.conversationStarters,
                "",
              ],
            },
            isFormDirty: true,
    ...
    createAgent: async (agentData: Partial<CreateAgentDto>, language: string) => {
      set({ isCreating: true });
      try {
      ...
      try {
        ...
        set((state) => {
          ...
          return {
            ...
            selectedAgentId: state.selectedAgentId === uuid ? null : state.selectedAgentId,
            previewMessages: [],
          };
        });
